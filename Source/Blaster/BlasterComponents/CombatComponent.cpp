...
void UCombatComponent::AttachActorToRightHand()
{
	if (Character == nullptr || Character->GetMesh() == nullptr || EquippedWeapon == nullptr) return;
...
void UCombatComponent::OnRep_CombatState()
{
	...
	case ECombatState::ECS_ThrowingGrenade:
		if (Character && !Character->IsLocallyControlled())
		{
			Character->PlayThrowGrenadeMongtage();
...
void UCombatComponent::ThrowGrenade()
{
	...
	if (Character && !Character->HasAuthority()) // 서버가 아닌곳에서 실행.
	{
		ServerThrowGrenade();
	}

	if (Character && Character->HasAuthority()) // 서버에서 실행
	{
...
void UCombatComponent::SetAiming(bool bIsAiming)
{
	if (Character == nullptr || EquippedWeapon == nullptr) return;
	bAiming = bIsAiming;
	// unreal will filter rpc only run on server if owner is client
...
void UCombatComponent::TraceUnderCrosshairs(FHitResult& TraceHitResult)
{
	...
	if (bScreenToWorld)
	{
		...
		{
			// if there is no collision impact point will be unexpected
...
void UCombatComponent::InitializeCarriedAmmo()
{
	...
	CarriedAmmoMap.Emplace(EWeaponType::EWT_GrenadeLauncher, StartingGrendeLauncherAmmo);
